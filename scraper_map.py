...
@dataclass
class WeatherData:
    """Holds the scraped weather data from the station detail list view."""

    # Meta
    station_id: str | None = None  # Added station ID if available
    station_name: str | None = None
    data_updated: datetime.datetime | None = None  # Parsed from string timestamp + timezone
    data_available: bool = False
    timezone_str: str | None = None  # Store the detected timezone string

    # Core Metrics (mapped from data-param attributes)
    air_density: float | None = None  # kg/m3
    barometric_pressure: float | None = None  # Station Pressure (mb)
    brightness: int | None = None  # lux
    delta_t: float | None = None  # °C
    dew_point: float | None = None  # °C
    feels_like: float | None = None  # °C
    humidity: float | None = None  # %
    heat_index: float | None = None  # °C
    lightning_strike_count: int | None = None  # Lifetime? Check context if needed
    lightning_strike_count_last_3hr: int | None = None
    lightning_strike_last_distance: str | None = None  # e.g., "29 - 33 km" - keep as string?
    lightning_strike_last_epoch: str | None = None  # String like "3 days" - needs parsing or keeping as str
    precip_accum_local_day: float | None = None  # mm (Today)
    precip_accum_local_yesterday: float | None = None  # mm (Yesterday)
    precip_minutes_local_day: int | None = None  # minutes (Today)
    precip_minutes_local_yesterday: int | None = None  # minutes (Yesterday)
    sea_level_pressure: float | None = None  # mb
    solar_radiation: int | None = None  # W/m2
    temperature: float | None = None  # °C
    uv: float | None = None  # UV Index
    wet_bulb_temperature: float | None = None  # °C
    wet_bulb_globe_temperature: float | None = None  # °C
    wind_avg: float | None = None  # km/h
    wind_chill: float | None = None  # °C
    wind_direction: int | None = None  # degrees
    wind_cardinal: str | None = None  # Cardinal direction (N, NE, E, etc.)
    wind_gust: float | None = None  # km/h
    wind_lull: float | None = None  # km/h

    # Derived/Calculated Metrics
    precipitation_rate: float | None = None  # mm/hr (Derived from 'precip' field)
    precipitation_description: str | None = None  # (Derived from 'precip' field)

    # Flags
    is_raining: bool = False
    is_freezing: bool = False
    is_lightning: bool = False  # Heuristic based on recent strike count
...
def _extract_list_data(list_ul: Tag) -> dict[str, Any]:
    ...
    for item in list_items:
        ...
        determined_key = param_key or (
            "timezone" if label_text == "timezone" else None
        )
...
def _map_raw_data_to_weather(raw_data: dict[str, Any], data: WeatherData) -> WeatherData:
    ...
    mapping = {
        ...
        "barometric_pressure": ("barometric_pressure", _safe_parse_float),
        "brightness": ("brightness", _safe_parse_int),
        ...
        "lightning_strike_count_last_3hr": ("lightning_strike_count_last_3hr", _safe_parse_int),
        "lightning_strike_last_distance": ("lightning_strike_last_distance", str),
        "lightning_strike_last_epoch": ("lightning_strike_last_epoch", str),
        "precip_accum_local_day": ("precip_accum_local_day", _safe_parse_float),
        "precip_accum_local_yesterday": ("precip_accum_local_yesterday", _safe_parse_float),
        "precip_minutes_local_day": ("precip_minutes_local_day", _safe_parse_int),
        "precip_minutes_local_yesterday": ("precip_minutes_local_yesterday", _safe_parse_int),
        "sea_level_pressure": ("sea_level_pressure", _safe_parse_float),
        "solar_radiation": ("solar_radiation", _safe_parse_int),
        "uv": ("uv", _safe_parse_float),
        ...
        "wind_avg": ("wind_avg", _safe_parse_float),
    ...
    if "timestamp" in raw_data and "timezone" in raw_data:
        ...
        data.data_updated = _parse_detailed_timestamp(
            raw_data["timestamp"],
            raw_data["timezone"]
        )
...
def _process_precipitation(precip_str: str) -> tuple[float | None, str, bool]:
    ...
    description = (
        desc_match.group(1).strip().capitalize() 
        if desc_match else precip_str.strip().capitalize()
    )
...
def _parse_html_content(html_content: str, station_id: str | None) -> WeatherData:
    ...
    try:
        ...
        data.is_lightning = (data.lightning_strike_count_last_3hr or 0) > 0
...
async def _wait_for_content(page: Page, container_selector: str, wait_timeout: int):
    ...
    await page.locator(timestamp_selector).wait_for(
        state="visible", 
...
async def scrape_weather_data(
    ...
    try:
        ...
        # Wait for content
        container_locator = await _wait_for_content(
            page, 
            container_selector, 
            wait_timeout
        )
